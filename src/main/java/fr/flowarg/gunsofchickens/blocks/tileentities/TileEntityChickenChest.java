...
public class TileEntityChickenChest extends TileEntityLockableLoot implements ITickable
{
    ...
    {
        return this.hasCustomName() ? this.customName : "container.chicken_chest";
    }
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        ...
        if (compound.hasKey("CustomChickenChestName", 8)) this.customName = compound.getString("CustomChickenChestName");
    }
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound compound)
    {
        ...
        if (compound.hasKey("CustomChickenChestName", 8)) compound.setString("CustomChickenChestName", this.customName);
    ...
    {
        return new ContainerChickenChest(playerInventory, this);
    }

    @Override
    public String getGuiID()
    {
        return References.MODID + ":chicken_chest";
    }
    ...
    public void update()
    {
        if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 20 == 0)
        {
            ...
            {
                if (entityPlayer.openContainer instanceof ContainerChickenChest)
                {
                    if (((ContainerChickenChest) entityPlayer.openContainer).getChestInventory() == this)
                    {
